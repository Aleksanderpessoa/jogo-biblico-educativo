// ============================================
// JOGO BÍBLICO EDUCATIVO - ATIVIDADE PARA CONHECIMENTO BÍBLICO
// ============================================
...
const CONFIG = {
    // Número de oportunidades por partida (altere aqui se desejar)
    OPORTUNIDADES_INICIAIS: 14,
    
    // Número de peças do quebra-cabeça (altere aqui se desejar)
    NUMERO_PECAS: 10,
    
    // Configuração do quebra-cabeça (5x2 ou 2x5)
    PUZZLE_COLUNAS: 5,
    PUZZLE_LINHAS: 2,
    
    // Tempo para mostrar feedback (em milissegundos)
    TEMPO_FEEDBACK: 2000,
    
    // Ativar sons (true/false)
    ATIVAR_SONS: true,
    
    // Ativar animações (true/false)
...
// Lista de imagens para o quebra-cabeça
// Você pode adicionar mais imagens na pasta /img
const imagensPuzzle = [
    "img/leao.jpg",
    "img/ovelha.jpg",
    "img/coelho.jpg",
    ...
    "img/lago.jpg",
    "img/floresta.jpg",
    "img/campo_flores.jpg",
    "img/ceu_azul.jpg"
];

// Imagem padrão caso não encontre as imagens
...
// Configuração dos sons (descomente quando tiver os arquivos)
const sons = {
    acerto: "sounds/acerto.mp3",
    erro: "sounds/erro.mp3",
    vitoria: "sounds/vitoria.mp3",
    clique: "sounds/clique.mp3"
};
...
function escolherImagemAleatoria() {
    // Tenta usar as imagens da lista, senão usa a imagem padrão
    if (imagensPuzzle.length > 0) {
        const indiceAleatorio = Math.floor(Math.random() * imagensPuzzle.length);
        return imagensPuzzle[indiceAleatorio];
    }
...
function atualizarPlacar() {
    document.getElementById('acertos').textContent = estadoJogo.acertos;
    document.getElementById('erros').textContent = estadoJogo.erros;
    document.getElementById('oportunidades').textContent = estadoJogo.oportunidades;
    document.getElementById('pecas-reveladas').textContent = `${estadoJogo.pecasReveladas}/${CONFIG.NUMERO_PECAS}`;
    
    // Adiciona efeito de animação nos valores que mudaram
    const valores = document.querySelectorAll('.placar-valor');
...
function prepararModalResultado(venceu) {
    ...
    if (venceu) {
        ...
        modalTitulo.style.animation = 'celebrar 1s ease';
    } else {
    ...
    document.getElementById('estatistica-acertos').textContent = estadoJogo.acertos;
    document.getElementById('estatistica-erros').textContent = estadoJogo.erros;
    document.getElementById('estatistica-pecas').textContent = `${estadoJogo.pecasReveladas}/${CONFIG.NUMERO_PECAS}`;
    document.getElementById('estatistica-oportunidades').textContent = CONFIG.OPORTUNIDADES_INICIAIS - estadoJogo.oportunidades;
}
...
// ============================================
// FUNÇÕES DE ESTATÍSTICAS E LOCAL STORAGE
// ============================================

// Carrega estatísticas salvas
function carregarEstatisticas() {
    const vitorias = localStorage.getItem('biblia_vitorias') || 0;
    const ultimaPontuacao = localStorage.getItem('biblia_ultima_pontuacao') || 0;
    
    // Você pode exibir essas informações na interface se desejar
    console.log(`Vitórias: ${vitorias}, Última pontuação: ${ultimaPontuacao}`);
}

// Salva estatísticas
function salvarEstatisticas(venceu) {
    if (venceu) {
        const vitorias = parseInt(localStorage.getItem('biblia_vitorias') || 0);
        localStorage.setItem('biblia_vitorias', vitorias + 1);
    }
    
    const pontuacao = estadoJogo.acertos;
    localStorage.setItem('biblia_ultima_pontuacao', pontuacao);
    
    const melhorPontuacao = parseInt(localStorage.getItem('biblia_melhor_pontuacao') || 0);
    if (pontuacao > melhorPontuacao) {
        localStorage.setItem('biblia_melhor_pontuacao', pontuacao);
    }
}
...
// Toca um som (se estiver ativado)
function tocarSom(tipoSom) {
    if (!CONFIG.ATIVAR_SONS) return;
    
    // Aqui você pode implementar a reprodução de sons
    // Por enquanto, apenas log no console
    console.log(`Tocando som: ${tipoSom}`);
    
    // Exemplo de implementação futura:
    /*
    const audio = new Audio(sons[tipoSom]);
...
// ============================================
// FUNÇÕES DE PERSONALIZAÇÃO (FÁCEIS DE ALTERAR)
// ============================================

/*
COMO PERSONALIZAR O JOGO:

1. Para alterar o número de oportunidades:
   Mude CONFIG.OPORTUNIDADES_INICIAIS no início do arquivo

2. Para alterar o número de peças:
   Mude CONFIG.NUMERO_PECAS no início do arquivo

3. Para adicionar novas perguntas:
   Adicione ao array perguntasBiblicas seguindo o formato:
   {
...
4. Para adicionar novas imagens:
   Adicione os arquivos na pasta /img e inclua o caminho em imagensPuzzle

5. Para adicionar novos sons:
   Adicione os arquivos na pasta /sounds e configure no objeto sons

6. Para desativar sons ou animações:
   Mude CONFIG.ATIVAR_SONS ou CONFIG.ATIVAR_ANIMACOES para false

7. Para alterar as cores e design:
   Modifique o arquivo style.css

8. Para alterar o tempo de feedback:
   Mude CONFIG.TEMPO_FEEDBACK (em milissegundos)
*/
